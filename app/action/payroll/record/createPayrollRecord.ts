...
function mapSalaryField(
  recordId: number,
  field: NewSalaryField,
): InferInsertModel<typeof payrollFieldValueTable> {
  return {
...
function mapOtField(
  recordId: number,
  field: NewOtField,
): InferInsertModel<typeof otFieldValueTable> {
  return {
    payrollRecordId: recordId,
    name: field.name,
    nameEng: field.nameEng,
...
function mapPenaltyField(
  recordId: number,
  field: NewPenaltyField,
): InferInsertModel<typeof penaltyFieldValueTable> {
  return {
    payrollRecordId: recordId,
    name: field.name,
    nameEng: field.name,
    ...
    const result = await globalDrizzle.transaction(async (tx) => {
      ...
      const [salaryFields, otFields, penaltyFields] = await Promise.all([
        tx
          ...
          .where(eq(salaryFieldsTable.shopId, periods[0].shopId)),
        tx
          .select()
          .from(otFieldsTable)
          .where(eq(otFieldsTable.shopId, periods[0].shopId)),
        tx
          .select()
          .from(penaltyFieldsTable)
          .where(eq(penaltyFieldsTable.shopId, periods[0].shopId)),
      ]);
      ...
      for (const record of inserted) {
        allSalaryValues.push(
          ...mapFields(record.id, salaryFields, mapSalaryField),
        );
        allOtValues.push(...mapFields(record.id, otFields, mapOtField));
        allPenaltyValues.push(
          ...mapFields(record.id, penaltyFields, mapPenaltyField),
        );
      }

      // Bulk insert per table
      if (allSalaryValues.length)
        await tx.insert(payrollFieldValueTable).values(allSalaryValues);
      if (allOtValues.length)
        await tx.insert(otFieldValueTable).values(allOtValues);
      if (allPenaltyValues.length)
        await tx.insert(penaltyFieldValueTable).values(allPenaltyValues);
