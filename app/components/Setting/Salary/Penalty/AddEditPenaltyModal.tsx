  ...
  const closeHandler = () => {
    ...
    const submitHandler = async(data:Omit<NewPenaltyField,"shopId">) => {
      ...
      try{
        ...
        let msg = err
        if(err.message== "ER_DUP_ENTRY") msg = "Penalty cannot have duplicate name"
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  ...
                  <FormControl>
                    ...
                    <ToggleButtonGroup
                      value={PENALTY_METHOD[field.value as keyof typeof PENALTY_METHOD]}
                      // size="sm"
                      onChange={(e,v)=>{
                      field.onChange(PENALTY_METHOD[v as keyof typeof PENALTY_METHOD])}}
                    >
                      <Button value={PENALTY_METHOD.PERMINUTE}>per minute</Button>
                      <Button value={PENALTY_METHOD.HOURLY}>hourly</Button>
                      <Button value={PENALTY_METHOD.DAILY}>daily</Button>
                    </ToggleButtonGroup>
                  </FormControl>
                )}
              />
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                 ...
                 <FormControl>
                  ...
                  <Select
                    value={PENALTY_TYPE[field.value as keyof typeof PENALTY_TYPE]}
                    onChange={(e,v)=>{
                      field.onChange(PENALTY_TYPE[v as keyof typeof PENALTY_TYPE])}}
                  >
                    <Option value={PENALTY_TYPE.BASEDONSALARY}>base on salary</Option>
                    <Option value={PENALTY_TYPE.CONSTANT}>constant</Option>
                  </Select>
                </FormControl>
                )}
              />
