...
export default function EmailForm({shopData}:EmailFormProps){
    ...
    const onSubmit = async (data:Omit<NewShop,"name">)=>{
        ...
        const emailVerify = await verify(data.SMTPHost||"",data.SMTPPort||0,data.SMTPSecure||true,data.emailAddress||"",data.emailPassword||"")
        ...
        <div className="flex flex-col ">
            <FormProvider {...methods}>
                <form onSubmit={(e) => {
                    ...
                    <div className="w-fit">
                        <Button loading={isSubmitting} disabled={isSubmitting} type="submit" color="primary" variant="outlined">
            ...
            <div hidden={!verified} className="flex flex-col gap-2 justify-end w-full mt-3">
                {verifyStatus ? <section className="w-[50%] bg-green-100 border border-green-600 px-4 py-2 rounded-md">
                    <span className="flex flex-row gap-1 items-center">
                        <Icon className="text-green-700" icon="lets-icons:check-fill" fontSize={"20"}/>
                        <p className="text-green-700 opacity-70">This email config is valid and ready to use</p>
                    </span>
                </section> :
                <section className="w-[50%] bg-red-100 border border-red-600 px-4 py-2 rounded-md">
                    <span className="flex flex-row gap-1 items-center">
                        <Icon className="text-red-700" icon="mdi:cross-circle" fontSize={"20"}/>
                        <p className="text-red-700 opacity-70">Cannot verify this email config. {verifyError}</p>
                    </span>
                </section>
                }
            </div>
