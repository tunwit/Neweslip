...
export default function Home() {
  ...
  if(!periodId) router.back()

  const { data,isLoading, isSuccess } = usePayrollRecords(Number(periodId));
  const randomColor = useMemo(() => getRandomPastelColor(), []);
  const queryClient = useQueryClient();
  const {data:stat } = usePayrollPeriodStats(Number(periodId))
  console.log(stat);
  
  const deleteHandler = async() =>{
    try{
      await deletePayrollRecords(checked,Number(periodId))
  ...
  return (
    <main className="min-h-screen w-full bg-white font-medium">
      <PayrollsAddEmployeeModal open={openAdd} setOpen={setOpenAdd} periodId={Number(periodId)}/>
      ...
      <div className="mx-10 flex flex-col min-h-screen ">
        ...
        <div className="mt-5 flex flex-row justify-between">
          <p className="text-black text-4xl font-bold">Payroll - {periodId?.padStart(3,"0")}</p>
          <Button
        ...
        <div className="mt-8 flex flex-row gap-2">
          ...
          <div className="w-[20%]">
            ...
            <BranchSelector branchId={branchId} onChange={setBranchId} isEnableAll={true}/>
          </div>
        </div>
        <div className="flex flex-col justify-center ">
          ...
          <div className="w-full border border-[#d4d4d4] rounded-sm max-h-[calc(100vh-300px)] overflow-x-auto overflow-y-auto shadow-sm">
            <TableWithCheckBox data={data?.data}
                ...
                setOpen={setOpenEdit}
                columns={[
                  { key: "avatar", label: "", width:"6%" ,render: (row) => (
                    <div
                  ...
                  { key: "name", label: "Name",render: (row) => `${row.employee.firstName} ${row.employee.lastName}`},
                  { key: "nickName", label: "Nick Name",render: (row) => `${row.employee.nickName}`, },
                  { key: "branch", label: "Branch",render: (row) => `${row.employee.branch}`, },
                  { key: "total", label: "Total",render: (row) => `${1000}`, },
