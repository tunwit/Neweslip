import { branchesTable, employeesTable, shopOwnerTable, shopsTable } from "@/db/schema";
import globalDrizzle from "@/lib/drizzle";
import { errorResponse, successPaginationResponse, successResponse } from "@/utils/respounses/respounses";
   ...
   const empStatus = request.nextUrl.searchParams.get("status");
    ...
    if (empStatus && empStatus !== "ALL") {
        if (empStatus in EMPLOYEE_STATUS) {
            validateEmpStatus = EMPLOYEE_STATUS[empStatus as keyof typeof EMPLOYEE_STATUS];
        } else {
    ...
    const totalItems = await globalDrizzle.select({ count: count() }).from(employeesTable);
    ...
    const employees: EmployeeWithShop[] = await globalDrizzle
       .select({
        ...
        .from(employeesTable)
        .innerJoin(shopOwnerTable, eq(employeesTable.shopId, shopOwnerTable.shopId))
        ...
        .where(and(
            ...
            ...(validateEmpStatus !== undefined ? [eq(employeesTable.status, validateEmpStatus)] : [])
        ))
